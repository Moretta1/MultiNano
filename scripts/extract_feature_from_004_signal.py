...
def interp(x):
    ...
    If the input list has fewer than 2 points, we pad/repeat it to 100 points to avoid interpolation errors.
    """
    l = len(x)
    if l == 0:
        return [0.0] * 100
        ...
        f = interpolate.interp1d(x_old, y, kind='slinear')
        x_new = np.linspace(0, l - 1, 100)
        y_new = f(x_new)
        return np.around(y_new, 4).tolist()
    ...
    with open(signal_file) as f:
        ...
        for line in f:
            try:
                ...
                raw_signal = [base_signal_string.split("*") for base_signal_string in
                              signal_string.split("|")]  # string list

                full_length_signal = np.array([x for x in re.split('\*|\|', signal_string)],
                                              dtype=int)  # for normlization
                ...
                for index in range(clip, len(sequence) - clip):
                    ...
                    if scaling == "min_max":
                        kmer_raw_signal = [
                            (x - full_length_signal_min) / (full_length_signal_max - full_length_signal_min) for x in
                            kmer_raw_signal]  # min_max scaling
                    elif scaling == "zscore":
                        kmer_raw_signal = [(x - full_length_mean) / full_length_std for x in
                                           kmer_raw_signal]  # z-score scaling
                    elif scaling == "median_mad":
                        kmer_raw_signal = [
                            (x - np.median(full_length_signal_uniq)) / robust.mad(full_length_signal_uniq) for x in
                            kmer_raw_signal]  # median_mad scaling
                    ...
                    line = "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" % (read_id,
                                                                                         ...
                                                                                         "|".join(
                                                                                             [str(x) for x in std]),
                                                                                         "|".join(
                                                                                             [str(x) for x in median]),
                                                                                         "|".join(
                                                                                             [str(x) for x in length]),
                                                                                         "|".join([str(x) for x in
                                                                                                   kmer_base_quality]),
                                                                                         "|".join([str(x) for x in
                                                                                                   kmer_raw_signal[0]]),
