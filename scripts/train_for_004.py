...
# RMs = ["m6A", "m1A", "m5C", "hm5C", "I", "m7G", "psi"] # RNA002 dataset
RMs = ["m6A", "m5C"]  # RNA004 dataset, can be extended to wilder ranges

num_task = len(RMs)num_task = len(RMs)

# this file is of similar content with train.py
# only input feature dictionary changed to RNA004 features
# please change to your own data path when trying

data_pth = '/data/rlwang/multinano/data/features/'

train_mod_dict = {
    "m5C": data_pth + 'm5C/m5c.train.tsv',
    "m6A": data_pth + 'm6A/m6A.train.tsv',
}

test_mod_dict = {
    "m5C": data_pth + 'm5C/m5c.test.tsv',
    "m6A": data_pth + 'm6A/m6A.test.feature.tsv',
}

valid_mod_dict = {
    "m5C": data_pth + 'm5C/m5c.valid.tsv',
    "m6A": data_pth + 'm6A/m6A.valid.feature.tsv',
}

train_unmod_dict = {
    "m5C": data_pth + 'C/unmod.c.train.tsv',
    "m6A": data_pth + 'A/unmod.a.train.tsv',
}

test_unmod_dict = {
    "m5C": data_pth + 'C/unmod.c.test.tsv',
    "m6A": data_pth + 'A/unmod.a.test.tsv',
}

valid_unmod_dict = {
    "m5C": data_pth + 'C/unmod.c.valid.tsv',
    "m6A": data_pth + 'A/unmod.a.valid.tsv',
}
...
# uncomment to try different structures
'''
...
def construct_data(RMs, mode, len_train=3e2, len_test=2e2, len_val=2e2):
    ...
    for i in range(len(RMs)):
        if mode == 'train':
            x, y = load_data(data_mod=train_mod_dict[RMs[i]], data_unmod=train_unmod_dict[RMs[i]], data_length=len_train)
        elif mode == 'test':
...
def train(model, train_loader, test_loader, args, lossWeight):
    ...
    for epoch in range(args.epoch):
        ...
        for i, (x, y) in enumerate(train_loader):
            ...
            y_pred = model(signal, kmer, mean, std, intense, dwell, base_quality) # for NaiveNet or SignalTransformer-v2
...
if __name__ == "__main__":
    ...
    parser.add_argument('--epoch', type=int, required=False, default=50, help='Training epoch')
    parser.add_argument('--bs', type=int, required=False, default=256, help='batch_size')
    parser.add_argument('--lr', type=float, required=False, default=0.00001, help='learning_rate')
    ...
    x_valid, y_valid, y_valid_df = construct_data(RMs, mode="valid", len_val=12e1)
